...
bool Engine::init () {
    ...
    EventDispatcher::Instance ().subscribe<ApplyMarchingCubesEvent> (
        [this](const ApplyMarchingCubesEvent& e) {
            handleApplyMarchingCubes (e);
        });
...
void Engine::handleApplyMarchingCubes (const ApplyMarchingCubesEvent& e) {
    m_renderer.getScene ().getAllNodes ()[0]->setScale (glm::vec3 (1.0f));
    m_renderer.getScene ().setFocusGeometry (
        std::make_shared<graphics::Geometry> (
            graphics::MarchingCubes::reconstructMesh (
